...
public class TFIDFCalculator {
	...
	public double calculate(String docContent, List<String> corpusList) {
		this.corpus = new ArrayList<>();
		for (String doc : corpusList) {
			addDocumentToCorpus(doc);
		}
		return calculateDocumentTfIdf(docContent);
	}
	...
	public double calculateDocumentTfIdf(String document) {
		...
		for (String word : tf.keySet()) {
			...
			// For words not in corpus, we'll use a smoothing value or 0
	...
	private Map<String, Double> calculateInverseDocumentFrequency() {
		...
		for (String word : idf.keySet()) {
			...
			// To avoid log(1) = 0 for words in every document, some use log(N / (df + 1)) +
			// 1
			// But simple log(N / df) is usually enough if we want non-negative.
			double df = idf.get(word);
			idf.put(word, Math.log((double) totalDocs / df));
		}
